...
** property.
** The sizes of servname and pwd are retained at the original size of
** CS_MAX_NAME (= CS_MAX_NAME_NO_LRGID).
*/
typedef struct srv_rmtpwd
{
	CS_INT	servnamelen;
	CS_BYTE	servname[CS_MAX_NAME_NO_LRGID];
	CS_INT	pwdlen;
	CS_BYTE	pwd[CS_MAX_NAME_NO_LRGID];
} SRV_RMTPWD;
...
/*
** The following constants are used for callback
** installation and for capability tests.
*/
...
#define	SRV_C_DBG_STACK		(CS_INT)10
#define	SRV_C_DBG_SWITCH	(CS_INT)11
...
** Note that we are overloading the status field. It is actually
** only two-bytes big. We are going to use the upper-bytes to indicate
** that this senddone should use a TDS_DONEPROC or TDS_DONEINPROC token
** instead of the TDS_DONE token.
*/
...
#define SRV_DONE_DONEPROC 	(CS_INT) 0x00010000
#define SRV_DONE_DONEINPROC 	(CS_INT) 0x00020000
...
#define SRV_S_RETPARMS		(CS_INT)348

#define SRV_S_NATIVESTACKSIZE	(CS_INT)349

#define SRV_S_LASTPROP		(CS_INT)349
...
#define SRV_T_RETPARMS		(CS_INT)260

#define SRV_T_LASTPROP		(CS_INT)260
...
/*
** Default stack size argument for srv_spawn. Instructs open server to
** use the the stack size set with srv_props().
*/
#define SRV_DEFAULT_STACKSIZE		(CS_INT)(-1)

/*
** SRV_MINSTACKSIZE is the minimum allowable stack size that can be configured.
** SRV_DEF_STACKSIZE is the default stack size.
*/
#if defined(__linux) && (defined(__ia64) || defined(__x86_64__))
#define SRV_MINSTACKSIZE		196608
#define SRV_DEF_STACKSIZE       	SRV_MINSTACKSIZE
#else
#define SRV_MINSTACKSIZE		0x8800
#define SRV_DEF_STACKSIZE       	65536
#endif

/*
** Minimum allowable network buffer size that can be configured
*/
#define SRV_MIN_MAXNETBUF		(CS_INT)512
...
#define SRV_DEF_NETBUFSIZE       	(CS_INT)8192
...
extern CS_RETCODE CS_PUBLIC
srv_preemptive_execution(CS_RETCODE (*funcp) (CS_VOID *), CS_VOID *arg);
...
extern CS_RETCODE CS_PUBLIC srv_timedsleep PROTOTYPE((
	CS_VOID *sleepevent,
	CS_CHAR *sleeplabel,
	CS_INT sleepflags,
	CS_INT *info,
