...
smc_output_connect PROTOTYPE ((
  ...
  SMC_CHARP		userPassword,	/* identifies a valid password for    */
					/*   the user login                   */
  SMC_CHARP     OutputDBName,  /* identifies a valid database name on the output ase */

  SMC_CHARP		interfaceFile,	/* explicit pathname of interfaces    */
					/*   file 	    	    	      */
  SMC_ERR_CALLBACK	OutputASEErrCallback,	/* error callback function address    */
					/*                                    */
  SMC_SIZET		packetSize,	/* Sets connection packet size	      */
					/*   (0 ==> Sybase default)	      */
  SMC_SIZET		loginTimeout,	/* Sets login timeout for server      */
					/*   connections (0 ==> default)      */
  SMC_CONNECT_ID	monitoringHandle, /* input value used to find the correct output connection*/
  SMC_CONNECT_IDP	clientIdHandle	/* returned value that identifies a   */
					/*   client's connection              */
  ));

extern SMC_RETURN_CODE CS_PUBLIC  
smc_prepare_output_view PROTOTYPE ((
  SMC_INT			DbSessionID ,  /* Id of the session in the Output database */
  SMC_CHARP         DbName,       /* name of the output database */
  SMC_CHARP			tableName,	/* output table name*/
  SMC_CHARP         ColumnsDDL,	/* DDL definition of output table columns  */
  SMC_CONNECT_ID       monitoringHandle /* input value used to find the correct output connection*/
...
extern SMC_RETURN_CODE CS_PUBLIC  
smc_update_output_catalog PROTOTYPE ((
  SMC_CHARP         DbName,       /* name of the output database */
  SMC_INT			clientID ,  /* HS client ID */                                       
  SMC_INT			sessionID ,  /* ID of the HS recording session */                                       
  SMC_INTP			DbSessionId,   /* corresponding ID of the session within the output ASE*/                                       
   SMC_CHARP         monAseName,   /* name of the monitored database */
  SMC_CHARP         hsName,    /* name of the monitoring Historical Server */
  SMC_CONNECT_ID       monitoringHandle  /* input value used to find the correct output connection*/
...
extern SMC_RETURN_CODE CS_PUBLIC  
smc_output PROTOTYPE ((
  SMC_CHARP SQLtxt,   /* SQL text output to insert into the output ASE */                                 
   SMC_CONNECT_ID       monitoringHandle  /* input value used to find the correct output connection*/
  ));
...
extern SMC_RETURN_CODE CS_PUBLIC  
smc_create_alarm_with_threshold PROTOTYPE ((
  SMC_CONNECT_ID	clientId,	/* identifies a client's connection   */
					/*                                    */
  SMC_VIEW_ID           viewId,		/* identifies a client's view         */
                                      	/*                                    */
  ...
  SMC_ALARM_CALLBACK	alarmCallback,  /* function to call to invoke the     */
					/*   alarm	    	    	      */
  SMC_ALARM_IDP		alarmId,	/* id returned by this function       */
  SMC_INT occurencesThreshold           /* number of times the condition must */
					/* be met per refresh before we fire  */
					/* an alarm, old behaviour corresponds*/
					/* to occurencesThreshold =0          */
  ));
